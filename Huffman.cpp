...
void Huffman::serialize_tree(HNode* node, std::string& s) {
        if (node) {
        // If the node is a leaf, append the character to the serialized string
        if (!node->left && !node->right) {
            s += node->value;
            s += "//"; //Null pointer represented for for both right and left by '//'
        } else {
            // Inner node, append '*' and recurse on left and right
            s += "*";
